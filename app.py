...
def make_bar_chart(ws, title, value_col, y_axis_title, style, last_row):
    """Crea un BarChart con categorías multi-nivel (Vehículo + Conductor)."""
    ch = BarChart()
    ch.type = 'col'
    ch.style = style
    ch.title = title
    ch.x_axis.title = 'Unidad / Conductor'
    ...
    # Categorías multi-nivel: cols 1 (Alias) + 2 (Conductor)
    cats = Reference(
        ws,
        min_col=1, max_col=2,
